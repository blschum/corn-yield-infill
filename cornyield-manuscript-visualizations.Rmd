...
bio_imp_plot <- ggplot(bio.imp, aes(x=reorder(varfull, VI), y=VI)) + 
  geom_point() +
  geom_segment(aes(x=varfull,xend=varfull,y=0,yend=VI)) +
  ylab("Decrease variance") +
  xlab("") +
  coord_flip() +
  theme_minimal()

plot_grid(bio_imp_plot, farmer_imp_plot, ncol = 2, labels = c("A.", "B."))
ggsave("./viz/Figure2.png", dpi = 400, width = 240, height  = 120, units = "mm", bg = "white")
```
...
jpeg(file = "viz/Figure3.png", width = 10, height = 6,
     units = "in", res = 600)
...
ggsave("./viz/Figure4.png", dpi = 400, width = 240, height  = 120, units = "mm", bg = "white")
...
f
tmap_save(tm = f, file = './viz/Figure5.png', dpi = 400, width = 8, height = 5)
...
ggsave(file = './viz/Figure6.png', dpi = 300, height = 220, width = 220,units="mm")
```
