...
b_sum <- b_infill %>% group_by(GEOID) %>% summarise(ave = mean(average_prediction))


b_sp <- merge(ctys, b_sum, by = "GEOID")

b <- tm_shape(b_sp) +
  tm_polygons(col = "ave", breaks = c(25,75,100,125,150,175,200,225), palette = "Oranges", border.col = "transparent", title = "Corn Yield\n(bu/acre)", legend.hist = TRUE) +
  tm_legend(outside = TRUE, hist.width = 1.5) +
  tm_layout(frame = FALSE,
            legend.hist.size = 0.5) +
    tm_shape(frr_final) +
...
f_sum <- f_infill %>% group_by(GEOID) %>% summarise(ave = mean(average_prediction))
...
diff <- tm_shape(diff_sp) +
  tm_polygons(col = "DIFF", breaks = c(-30,-10,-5,-2.5,0,2.5,5,10,20,45), palette = c("#B2182B", "#D6604D", "#F4A582", "#FDDBC7", "#D1E5F0" ,"#92C5DE","#4393C3", "#2166AC", "#053061"), border.col = "transparent", title = "Average Difference\n(bushels/acre)", legend.hist = TRUE, title = title) +
  tm_legend(outside = TRUE, hist.width = 1.5) +
  tm_layout(frame = FALSE,
            legend.hist.size = 0.5) +
      tm_shape(frr_final) +
...
perc_infill <- diff_infill %>% dplyr::filter(DIFF > -10 & DIFF < 10) #11366/14631 = 77.7%
