...
Default mtry = p/3 (so 19/3); but tuning suggests that mtry = 11 minimizing cross-validated RMSE!
```{r evaluate = FALSE}
...
jpeg("./viz/rf-tune-mtry.jpeg", height = 4, width = 6, units = "in", res = 400)
plot(rf_gridsearch)
dev.off()

ggsave(plot=rf_tune_mtry, "./viz/rf-tune-mtry.jpeg", dpi = 400, device = 'jpeg')
...
# RF variable importance with various mtry levels
```{r evaluate = FALSE}
# remove variables that contributed least to the full model
