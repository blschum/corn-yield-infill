...
# subset for biophysical AND farm(er) variables
infill_farmer <- test_fulset %>% select(YEAR,YIELD,PERC_IRR,FRR,GDD,BV2,BV18,SDI_CDL_AG,BV9,BV4,TP,ELEVATION,S_PH_H2O,SLOPE,BV8,BV19,T_CEC_SOIL,BV15,T_REF_BULK_DENSITY,T_OC,29:39)
```
...
for (iter in 1:100){
  ...
  # performance on test set
  RMSE <- caret::RMSE(farmer_test$PREDS, farmer_test$YIELD)
  R2 <- 1 - (sum((farmer_test$YIELD - farmer_test$PREDNS)^2) / sum((farmer_test$YIELD - mean(farmer_test$YIELD))^2))
  MAE <- mae(farmer_test$PREDS, farmer_test$YIELD)
  MAPE <- MAPE(farmer_test$PREDS, farmer_test$YIELD)
  ...
  saveRDS(infill_bioph, file = paste0('./results/ranger-infill/farmer_infill_',iter,'.RDS'))
