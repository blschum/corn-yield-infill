---
title: "Variable Selection: RFs w/biovars, HWSD, & CDL"
...
yield <- yield %>% dplyr::select(-c("BV3", "BV7"))
...
See variables removed by KS/EB that I did not remove, below:
...
```{r}
yield_keep <- yield %>% dplyr::select(c(GEOID,YEAR,YIELD,CORN_SQKM,AG_SQKM,SDI_CDL_AG,GDD,TP,BV2,BV4,BV8,BV9,BV15,BV18,BV19,SLOPE,ELEVATION,S_PH_H2O,T_CACO3,T_CASO4,T_CEC_CLAY,T_CEC_SOIL,T_ESP,T_GRAVEL,T_OC,T_REF_BULK_DENSITY,T_SILT,T_ECE,T_SAND,PERC_IRR,FRR))

saveRDS(yield_keep, "./out/yieldRF.RDS")
```

### Pair-wise Correlation Values

```{r}
red <- as.data.frame(as.table(cor(yield_keep[4:30]), 1))
...
corr_yield <- round(cor(yield_keep[c(4:30)]), 2)
p.mat_yield <- cor_pmat(yield_keep[c(4:30)])
