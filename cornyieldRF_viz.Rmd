...
First, we'll work with the reduced RF modeling across all FRRs and years! Then, we'll visualize error over time using the individual year-by-year and FRR-by-FRR RFs!
...
# build GEOID back into RFpreds_red.RDS dataframe
red <- readRDS('./results/RFpreds_red.RDS')
corn_yield <- readRDS("./data/yieldRF.RDS")
ctys <- readRDS("./data/county.RDS")
state <- readRDS("./data/states.RDS")

# keep only counties with at least 3 years of data
sub_id <- corn_yield %>% mutate(count = 1) %>%  group_by(GEOID) %>% summarise(n = sum(count)) %>% filter(n >= 3)

corn_yield <- corn_yield %>% filter(GEOID %in% sub_id$GEOID)
...
red <- merge(red, yield_RF_reduced, by = c("YEAR","YIELD","PERC_IRR","CORN_SQKM","FRR","GDD","BV18","BV2","SDI_CDL_AG","AG_SQKM","BV9","TP","BV4","ELEVATION","BV8","S_PH_H2O","BV19","T_CEC_SOIL","SLOPE","BV15","T_OC","T_REF_BULK_DENSITY"))
...
diff <- red %>% dplyr::select(c(GEOID, YEAR, YIELD, PREDICTIONS)) %>% mutate(diff = PREDICTIONS - YIELD)
diff_sp <- merge(ctys, diff, by = "GEOID", duplicateGeoms = TRUE)

```

# Full Panel, reduced-RF

# Let's visualize observations v. predictions
```{r}
cor(red$YIELD, red$PREDICTIONS, method = "pearson")

p <- ggplot(red, aes(x=YIELD, y=PREDICTIONS)) +
  geom_point(size = 0.5) + 
  ...
  xlim(1,250) +
  ylim(0,250) +
  annotate("text", x = 175.0, y = 40.0, label = "RMSE = 16.9 bushels/acre\nVariance Explained = 81.6%\nr = 0.91", hjust=0)
p

ggsave(filename = "./viz/sp_reducedRF/cor_plot.png", plot = p, dpi = 300, width = 7, height = 7)
```
...
tmap_save(map, file = paste0('./viz/yield.png'))
```
